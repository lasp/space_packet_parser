    ...
    def _flatten_container(container: containers.SequenceContainer):
        for entry in container.entry_list:
            if isinstance(entry, containers.SequenceContainer):
                assert id(entry) in def_cont_ids, f"{entry.name} object not in def.containers"
                _flatten_container(entry)
            else:
                assert id(entry.parameter_type) in def_param_type_ids, (
                    f"{entry.parameter_type.name} not in definition.parameter_types"
                )
                assert id(entry) in def_param_ids, f"{entry.name} not in definition.parameters"
