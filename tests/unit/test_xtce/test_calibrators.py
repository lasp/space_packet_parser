            ...
            calibrators.ContextCalibrator(
                match_criteria=[
                    comparisons.Comparison(
                        required_value="678",
                        referenced_parameter="EXI__FPGAT",
                        operator=">=",
                        ...
                        expression=comparisons.Anded(
                            conditions=[
                                comparisons.Condition(
                                    left_param="P1", operator="==", right_value="100", right_use_calibrated_value=False
                                ),
                                comparisons.Condition(
                                    left_param="P4", operator="!=", right_value="99", right_use_calibrated_value=False
                                ),
                            ],
                            ors=[],
                        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                ...
                match_criteria=[
                    comparisons.BooleanExpression(
                        expression=comparisons.Anded(
                            conditions=[
                                comparisons.Condition(
                                    left_param="P1", operator="==", right_value="700", right_use_calibrated_value=False
                                ),
                                comparisons.Condition(
                                    left_param="P2", operator="!=", right_value="99", right_use_calibrated_value=False
                                ),
                            ],
                            ors=[],
                        )
            ...
            {"P1": common.FloatParameter(700.0, 100.0), "P2": common.FloatParameter(700.0, 99)},
            42,
            True,
            63.5,
        ),
                    ...
                    comparisons.BooleanExpression(
                        expression=comparisons.Ored(
                            conditions=[  # Neither of these are true given the parsed data so far
                                ...
                                comparisons.Condition(
                                    left_param="P2", operator="!=", right_value="700", right_use_calibrated_value=False
                                ),
                            ],
                            ands=[],
                        )
...
def test_spline_calibrator(elmaker, xtce_parser, xml_string: str, expectation):
    ...
    if isinstance(expectation, Exception):
        ...
        result_string = ElementTree.tostring(result.to_xml(elmaker=elmaker), pretty_print=True).decode()
        full_circle = calibrators.SplineCalibrator.from_xml(ElementTree.fromstring(result_string, parser=xtce_parser))
        assert full_circle == expectation
...
def test_spline_calibrator_calibrate(xq, order, extrapolate, expectation):
    ...
    spline_points = [
        calibrators.SplinePoint(-1.0, 0.0),
        calibrators.SplinePoint(0.0, 3.0),
        calibrators.SplinePoint(2.0, 2),
    ]
...
def test_polynomial_calibrator_calibrate(xq, expectation):
    ...
    polynomial_coefficients = [
        ...
        calibrators.PolynomialCoefficient(1.0, 2),
    ]
