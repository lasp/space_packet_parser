...
if __name__ == "__main__":
    packet_file = Path("../tests/test_data/ctim/ccsds_2021_155_14_39_51")
    packet_definition_file = Path("../tests/test_data/ctim/ctim_xtce_v1.xml")
    ...
    with packet_file.open("rb") as binary_data:
        ...
        list_comp_packets = [pkt for pkt in ccsds_generator if pkt.apid == apid]
        print(f"Found {len(list_comp_packets)} packets with APID {apid}")

        # Using a python filter function
        binary_data.seek(0)
        ccsds_generator = ccsds.ccsds_generator(binary_data)
        filtered_packets = filter(lambda pkt: pkt.apid == apid, ccsds_generator)
        print(f"Found {len(list(filtered_packets))} packets with APID {apid}")
        ...
        for packet_bytes in ccsds_generator:
            if packet_bytes.apid == apid:
                pkt_count += 1
        print(f"Found {pkt_count} packets with APID {apid}")
    ...
    ds = create_dataset(
        ...
        packet_filter=lambda pkt: pkt.apid == apid,
    )
